...
import { createWasmoonScripting } from '../../src/scripting/index.js';
...
function fixture(t, options = {}, engineOptions = {}) {
  ...
  const scripting = createWasmoonScripting(lua, options);
...
describe('Wasmoon scripting', () => {
  ...
  it('owns a single server and closes idempotently', () => {
    const plugin = createWasmoonScripting(lua);
    const owner = {};
    plugin.attach(owner);
    plugin.attach(owner);
    ...
    assert.throws(() => createWasmoonScripting(lua, { timeoutMs: 0 }), /positive/);
  });
